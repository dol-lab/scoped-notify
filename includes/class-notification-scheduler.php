...
class Notification_Scheduler {
    ...
    public function get_user_schedule(int $user_id, int $blog_id, string $channel): string {
        ...
        try {
            ...
            if (!empty($db_schedule)) {
                ...
                $this->logger->debug("Found schedule '{$schedule}' for user {$user_id}, blog {$blog_id}, channel '{$channel}'.", [
                    'user_id' => $user_id, 'blog_id' => $blog_id, 'channel' => $channel, 'schedule' => $schedule
                ]);
            } else {
                 $this->logger->debug("No specific schedule found for user {$user_id}, blog {$blog_id}, channel '{$channel}'. Using default.", [
                    'user_id' => $user_id, 'blog_id' => $blog_id, 'channel' => $channel
                ]);
            }
        } finally {
    ...
    public function calculate_scheduled_send_time(string $schedule_type): ?string {
        ...
        if (!$wp_timezone) {
            ...
            try {
                $wp_timezone = new DateTimeZone($offset_string);
                 ...
                 $this->logger->error("Invalid timezone offset calculated: {$offset_string}. Falling back to UTC.", ['gmt_offset' => $gmt_offset]);
        ...
        try {
            if ($schedule_type === 'daily') {
                ...
                // it should be correct. If it resulted in a past date (shouldn't happen with 'next'), add a week.
                 if ($scheduled_time_local <= $now) {
                     ...
                     $this->logger->debug("Weekly schedule calculation resulted in past/present time, adjusting to next week.", [
                         'now' => $now->format(DateTime::ATOM), 'calculated' => $scheduled_time_local->format(DateTime::ATOM)
                     ]);
                     $scheduled_time_local->modify('+1 week');
                 }
            ...
            $this->logger->debug("Calculated scheduled send time.", [
            ...
            return $mysql_utc_time;
             ...
             $this->logger->error("Error calculating scheduled send time: " . $e->getMessage(), [
                'schedule_type' => $schedule_type, 'wp_timezone' => $wp_timezone->getName(), 'exception' => $e
             ]);
